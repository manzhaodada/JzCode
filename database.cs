...
namespace JZ计算机软件开发语言
{
    ...
    {
        static String[] englishData = {"public class", "public " +
        ...
        static String[] chineseData = {"#主类", "#程序入口","#公开",
            "#私有","#字节型","#字符型","#整数型","#长整数型"
                ,"#单精度小数型","#双精度小数型","#文本型",
            "#布尔型","#如果","#否则如果","#否则",
            "#判断后循环","#变量循环","#打印并换行","#打印"
        ...
        public static string translateJzCode(string lineJzCode)
        {
            ...
            {
                string temp = chineseData[i];
                bool state = lineJzCode.Contains(temp);
                if (state == true)
                {
                    temp = englishData[i];
                    lineJzCode = lineJzCode.Replace(chineseData[i], temp);
                }
            }
            return lineJzCode;
        }
